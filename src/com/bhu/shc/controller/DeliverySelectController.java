...
/**
 * Servlet implementation class DeliverySelectController
 */
@WebServlet("/delivery/select.do")
public class DeliverySelectController extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public DeliverySelectController() {
        super();
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		List<Delivery> list = deliveryService.selectById(deliveryId);
		HttpSession httpSession = request.getSession();
		
		//查询成功 跳转到新的商品信息界面  查询失败转发回到商品信息页面
		if(list.size() == 0) {
//			request.setAttribute("selecttip", "查询失败!");
			httpSession.setAttribute("deliverySelectTip", "查询失败!");
